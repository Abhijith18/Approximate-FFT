...
def inttobyte(inp):
    ...
    return [lsb,msb]
def bytestoint(inp):
    binary = bin(inp[0])[2:].zfill(8) + bin(inp[1])[2:].zfill(8) + bin(inp[2])[2:].zfill(8) 
    if binary[0] == '1':
...
for i in range(1000000):
    x = random.randint(-129,128)
    y = random.randint(-129,128)
    print(i)
    [lsb,msb] = inttobyte(x)
    ...
    it=(ComPort.read(3))                #for receiving data from FPGA
